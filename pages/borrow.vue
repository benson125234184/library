<template>
    <div class="p-6">
      <h1 class="text-2xl font-bold mb-4">借閱紀錄</h1>
      <div class="space-y-4">
        <!-- 控制面板 -->
        ...
        <Table v-if="viewMode === 'table'">
          ...
          <TableBody>
            <TableRow v-for="(book, index) in paginatedBooks" :key="index">
              ...
              <TableCell>
                <Button @click="reserveBook(index)">續借</Button>
              </TableCell>
            </TableRow>
          </TableBody>
        </Table>
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          <div v-for="(book, index) in paginatedBooks" 
            ...
            <div class="p-4 space-y-2">
              ...
              <Button class="w-full" @click="reserveBook(index)">續借</Button>
            </div>
          </div>
        </div>
  ...
  function reserveBook(index) {
    ...
    const book = sortedBooks.value[actualIndex]
    const originalIndex = borrowedBooks.value.findIndex(
      item => item.title === book.title && item.borrowDate === book.borrowDate
    )
    
    borrowedBooks.value[originalIndex].dueDate = newDueDate.toISOString().split('T')[0]
  
    toast({
      title: '預約成功',
      description: `《${book.title}》已延長到 ${newDueDate.toISOString().split('T')[0]}`
    })
  }
